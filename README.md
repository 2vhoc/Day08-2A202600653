...
- [x] File `group_project/evaluation/eval_pipeline.py` — script RAGAS-first, có offline fallback
...
Task 5 Semantic     Task 6 BM25 lexical
        ...
        └──────┬────────┘
               ▼
Task 7 RRF / rerank fallback
               │
               ▼
Task 9 Hybrid retrieval + PageIndex fallback
               │
               ▼
Task 10 Generation with citation
        ...
        ┌──────┴────────┐
        ▼               ▼
app.py                  group_project/evaluation/eval_pipeline.py
Web chatbot demo        Offline evaluation + A/B report
```
...
|-----------|------|----------|------------|
| Vũ Văn Học | N/A | Tasks 1-3: data collection, crawl, markdown conversion | Hoàn thành |
| Vũ Văn Học | N/A | Tasks 4-6: chunking, indexing, semantic/BM25 retrieval | Hoàn thành |
| Vũ Văn Học | N/A | Tasks 7-10: reranking, fallback retrieval, generation citation | Hoàn thành |
| Vũ Văn Học | N/A | Group web chatbot + evaluation pipeline + report | Hoàn thành |
...
# Nếu muốn chạy RAGAS thật, set OpenAI key hợp lệ trong .env:
# OPENAI_API_KEY=...
...
Web chatbot chạy tại `http://127.0.0.1:8501`.
...
Repo đã có đủ file để deploy Railway:
...
Railway sẽ cấp biến `PORT` tự động. `app.py` đã đọc `PORT` và bind
`0.0.0.0` khi chạy production; endpoint healthcheck là `/api/health`.

Lệnh chạy local vẫn là:
...
Biến môi trường optional trên Railway:
...
Không set API key thì app vẫn chạy bằng fallback local từ dữ liệu đã index.
